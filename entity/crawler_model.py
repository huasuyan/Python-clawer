...
class ReportResult(Base):
    ...
    region_distribution_list = Column(JSON, nullable=True, comment="地域分布列表JSON")
    hot_analysis_words_list = Column(JSON, nullable=True, comment="热分析词列表JSON")
    hot_information = Column(Text, nullable=True, comment="热门信息")
    hot_information_list = Column(JSON, nullable=True, comment="热门信息列表JSON")
